...
# def query_pdf_database(query, collection_name):
#     """Query the specific ChromaDB collection for relevant chunks and store user history."""
#     try:
#         collection = client.get_collection(name=collection_name, embedding_function=embedding_function)
#     except Exception:
#         return f"Error: Collection '{collection_name}' not found. Please upload and process the PDF first."
...
# def query_pdf_database(query, collection_name):
#     """Query the specific ChromaDB collection for relevant chunks."""
#     try:
...
#             results.append(f"✅ Processed '{filename}': {num_chunks} chunks stored in '{collection_name}'.")
#         except Exception as e:
#             results.append(f"⚠️ Error processing '{filename}': {e}")
...
# def process_and_store_pdf(pdf_path, collection_name):
#     """Process the PDF, extract text, and store it in a dedicated ChromaDB collection."""
...
#     # Create a dedicated collection for this PDF
#     collection = client.get_or_create_collection(name=collection_name)
...
#     return len(chunks)  # Return the number of processed chunks


# def query_pdf_database(query, collection_name):
#     """Query the specific ChromaDB collection for the uploaded PDF."""
#     try:
#         # Get the collection for the specific PDF
#         collection = client.get_collection(name=collection_name)
#     except Exception as e:
#         return f"Error: Unable to find collection for {collection_name}. Make sure the PDF has been processed."

#     results = collection.query(
#         query_texts=[query],
#         n_results=3  # Fetch the top 3 most relevant documents
#     )
...
# def build_gradio_app():
#     with gr.Blocks() as app:
#                 gr.Markdown("### Upload your PDF for Analysis")
...
#                     # Ensure the name is within the length limits and valid
#                     return sanitized[:63].strip("_")
                
#                 def process_pdf(file):
#                     """Process the uploaded PDF and create a collection."""
#                     if not file:
#                         return "No file uploaded."
...
#                     if len(collection_name) < 3:
#                         return "Collection name derived from the PDF file is too short. Please rename your file."             
#                     try:
#                         num_chunks = process_and_store_pdf(pdf_path, collection_name)
#                         return f"Successfully processed {num_chunks} chunks into collection '{collection_name}'."
#                     except ValueError as e:
#                         return f"Error: {e}"
#                     except Exception as e:
#                         return f"Unexpected error: {e}"
            
#                 def query_uploaded_pdf(query, file):
#                     if not file:
#                         return "No file uploaded for query."
#                     pdf_path = file.name
